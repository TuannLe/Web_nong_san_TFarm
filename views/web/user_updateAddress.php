...
if (!isset($_GET['Address'])||!isset($_GET['idAccount'])) {
    echo "<script>window.location = 'user_address.php'</script> ";
} else {
    $idAddress = filter_input(INPUT_GET, 'Address');
    $idAccount = filter_input(INPUT_GET, 'idAccount');
}


if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $phone = filter_input(INPUT_POST, 'numberPhone');
    $addNew = filter_input(INPUT_POST, 'addr');
    $status = filter_input(INPUT_POST, 'status');
    $name = filter_input(INPUT_POST, 'name');

    $alert = $class->update_address($addNew, $status, $phone, $name, $idAddress);
}
?>
                    ...
                    <div class="col l-10 m-9 c-12">
                        <div class="user-container">
                            ...
                            <form action="user_updateAddress.php" method="POST">
                                <?php
                                $detailInfo = $class->get_address_where($idAddress, $idAccount);
                                if ($idAddress) {
