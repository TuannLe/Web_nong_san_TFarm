        ...
        ?>
                <div class="user-order-body-container">
                    ...
                    <div class="user-order-body-info">
                        ...
                        <?php
                        $check = $feedback->checkFeedback($item['IdOrder'], Session::get('userId'), $item['IdProduct']);
                        if(!$check){
                        ?>
                        <button onclick="window.location.href='evaluate.php?IdProduct=<?php echo $item['IdProduct'] ?>&IdOrder=<?php echo $item['IdOrder'] ?>'" class="button_evaluate js-action">Đánh giá</button>
                        <?php
                        } else {
                            echo '<p style=" color: var(--primary-color)">Sản phẩm đã được đánh giá</p>';
                        }
                        ?>
                    </div>
